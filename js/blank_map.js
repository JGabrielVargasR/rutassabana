...
function loadRoute(id, boundary){
	...
	var paradas = execute_query(db_path,"select AsGeoJSON(pt_parada.geom), pt_parada.id_parada as id_parada, pt_parada.nombre as nombre, direccion, p_municipio.nombre as municipio from pt_parada, p_municipio, t_ruta_parada where pt_parada.id_parada = t_ruta_parada.id_parada and id_mpio = mpio_divip and  t_ruta_parada.id_ruta= '"+id+"'",capas,'Paradas',['#query-output',false],stop);
