...
function getLocation(){
	...
	if (navigator.geolocation) {
		...
		function onLocationFound(e) {
			...
			var closest_stops = execute_query(db_path,"select asGeoJSON(pt_parada.geom), id_parada, pt_parada.nombre as nom_parada, direccion , p_municipio.nombre as nom_mpio from pt_parada, p_municipio where id_mpio = mpio_divip and st_intersects(pt_parada.geom, st_buffer(makepoint("+lon+","+lat+", 4326), 0.00903568))",capas,'Paradas',['#query-output',false],stopIcon);
			};
...
function loadPoint(id, latitude, longitude){
	capas.clearLayers();
	var parada = execute_query(db_path,"select asGeoJSON(geom), id_parada, pt_parada.nombre as nom_parada, direccion , p_municipio.nombre as nom_mpio from pt_parada, p_municipio where id_mpio = mpio_divip and id_parada = '"+id+"'",capas,'Parada',['#query-output',false],stopIcon);
